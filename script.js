...
window.addEventListener('load', () => {
    ...
    setTimeout(() => {
        ...
        // DISABILITA COMPLETAMENTE SPLINE SU MOBILE
        const isMobile = window.innerWidth <= 768;
        if (!isMobile) {
            splineIframe.style.opacity = '1';
        } else {
            // Su mobile: nascondi e disabilita tutti gli eventi
            splineIframe.style.display = 'none';
            splineContainer.style.pointerEvents = 'none';
            splineContainer.style.touchAction = 'none';
        }
    }, 2000);
});
...
function closeModalFunc() {
    ...
    // FIX: Forza il ripristino dello scroll su mobile
    const isMobile = window.innerWidth <= 768;
    if (isMobile) {
        ...
        // Forza il browser a ricalcolare lo scroll
        setTimeout(() => {
